...
function 超鬼王界面.to战斗准备界面()
	...
	if 操作.识别2(标识.超鬼王界面_自己发现标识) then
		for i=1,1 do
			if 操作.识别2(标识.超鬼王界面_六星超鬼王（位置1）) then
				鬼王星级=6
				break
			end
			if 操作.识别2(标识.超鬼王界面_五星超鬼王（位置1）) then
				鬼王星级=5
				break
			end
			if 操作.识别2(标识.超鬼王界面_四星超鬼王（位置1）) then
				鬼王星级=4
				break
			end
			if 操作.识别2(标识.超鬼王界面_三星超鬼王（位置1）) then
				鬼王星级=3
				break
			end
			if 操作.识别2(标识.超鬼王界面_二星超鬼王（位置1）) then
				鬼王星级=2
				break
			end
			if 操作.识别2(标识.超鬼王界面_一星超鬼王（位置1）) then
				鬼王星级=1
				break
			end
		end
	...
	if 鬼王星级>0 then
		...
		if 鬼王星级>=参数.自己发现的超鬼王最低打几星 and 鬼王星级<=参数.自己发现的超鬼王最高打几星 then
			攻打自己发现的=true
			当前血线=超鬼王界面.血线识别({134,282,400,298})
		end
