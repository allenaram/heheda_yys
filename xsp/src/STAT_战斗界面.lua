...
function 战斗界面.to战斗胜利界面()
	...
	if (参数.交叉寮突破 and not(参数.寮突已攻破) and 参数.主任务 and (mTime()-参数.寮突破开始计时时刻)>参数.寮突破间隔) or 
	   (参数.交叉死神 and 参数.主任务 and (mTime()-参数.死神开始计时时刻)>1800000) or 
	...
	while true do
		...
		if (参数.任务=='大蛇' and 参数.御魂点怪 or 
			参数.任务=='修罗战场' and 参数.修罗战场_点怪 or
			参数.任务=='业原火' and 参数.业原火_点怪) and 
			参数.主任务 and not 已点怪 then
			mSleep(600)
		else
			mSleep(1500)
		end
...
function 战斗界面.标记(已点怪, 开局怪未点)
	...
	if 参数.任务=='业原火' and 参数.业原火_点怪 and 参数.主任务 and not 已点怪 then
		if not 操作.识别2(标识.战斗界面_回目) then
			return false
		end
		参数.回目数 = 参数.回目数 + 1
...
function 战斗界面.标记己方(己方已点)
	if 己方已点 or 操作.识别2(标识.战斗界面_己方已点) then
		return true
	end
	...
	for i = 1,1 do
		...
		if 参数.任务=="御灵" and 参数.主任务 and 参数.御灵_标记己方~=0 and not 己方已点 then
			local tTarget = 
			{
				["1号位"] = 按钮.战斗界面_御灵_己方1号位,
				["2号位"] = 按钮.战斗界面_御灵_己方2号位,
				["3号位"] = 按钮.战斗界面_御灵_己方3号位,
				["4号位"] = 按钮.战斗界面_御灵_己方4号位,
				["5号位"] = 按钮.战斗界面_御灵_己方5号位,
				["左前"] = 按钮.战斗界面_御灵_己方左前,
				["中前"] = 按钮.战斗界面_御灵_己方中前,
				["右前"] = 按钮.战斗界面_御灵_己方右前,
				["左后"] = 按钮.战斗界面_御灵_己方左后,
