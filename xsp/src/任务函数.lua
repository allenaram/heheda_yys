...
function 执行任务.设置流程()
	...
	elseif 参数.任务=='探索' and  参数.探索组队类型~='队员' then -- 探索-单人/队长
		...
		elseif 参数.探索组队类型=='队长' then
			探索准备界面.Next=探索邀请界面.to探索关卡界面
			探索邀请界面.Next=探索关卡界面.to战斗准备界面
			探索关卡退出界面.Next=重邀界面.to探索邀请界面
			重邀界面.Next=探索邀请界面.to探索关卡界面
		end
	...
	elseif 参数.任务=='探索' and  参数.探索组队类型=='队员' then -- 探索-队员
		执行任务.重新识别=执行任务.探索队员
		执行任务.主任务=执行任务.探索队员
		执行任务.等待邀请.Next=探索关卡界面.to战斗准备界面（队员）
		探索关卡界面.Next=战斗准备界面.to战斗界面
		战斗准备界面.Next=战斗界面.to战斗胜利界面
		战斗界面.Next=战斗胜利界面.to战斗结算界面2
		战斗胜利界面.Next=战斗结算界面2.to探索关卡界面
...
--	table.insert(目标星级_自己发现,按钮.发现六星超鬼王)
	local 目标星级_好友邀请={}
	local 非目标星级_好友邀请={}
	if (1>=参数.好友邀请的超鬼王最低打几星 and 1<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现一星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现一星超鬼王) end
	if (2>=参数.好友邀请的超鬼王最低打几星 and 2<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现二星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现二星超鬼王) end
	if (3>=参数.好友邀请的超鬼王最低打几星 and 3<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现三星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现三星超鬼王) end
	if (4>=参数.好友邀请的超鬼王最低打几星 and 4<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现四星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现四星超鬼王) end
	if (5>=参数.好友邀请的超鬼王最低打几星 and 5<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现五星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现五星超鬼王) end
	if (6>=参数.好友邀请的超鬼王最低打几星 and 6<=参数.好友邀请的超鬼王最高打几星) then table.insert(目标星级_好友邀请,按钮.发现六星超鬼王) else table.insert(非目标星级_好友邀请,按钮.发现六星超鬼王) end
...
--				end
				if 参数.探索优先接受邀请 then
					if 操作.识别点击(按钮.接受邀请按钮,true,5,5) then 
						sysLog('接受邀请')
						参数.开始等待队友时刻=-1
						参数.已检查探索宝箱=false
						mSleep(1500)
	...
	if not(参数.默认接受邀请) or 参数.狗粮未换 then
		...
		for i=1,2 do
			if 操作.或识别点击({按钮.默认接受邀请按钮,按钮.接受邀请按钮},true,5,5) then
				sysLog('接受邀请')
				参数.开始等待队友时刻=-1
				参数.已检查探索宝箱=false
				mSleep(1500)
