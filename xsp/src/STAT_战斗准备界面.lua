...
function 战斗准备界面.to战斗界面()
	...
	while true do
		if 操作.识别2(标识.战斗准备界面_未准备标识) then
			...
			if 参数.御魂换狗粮 and (参数.任务=='大蛇' or 参数.任务=='业原火') and 参数.主任务 then
				战斗准备界面.换狗粮（御魂）()
			end
...
function 战斗准备界面.换狗粮（御魂）()
	...
	keepScreen(true)
	if 参数.狗粮表.左一 then
		local 左一满级标识={0x342312,"2|0|0xff9d1a,5|0|0x2d1f12,9|1|0xffa219,12|1|0xffa219,16|0|0x2a1d12,1|6|0xfbba11,9|5|0xffb713,13|4|0xffb114,2|12|0xefd207,6|11|0xfcd908,11|10|0xcfad0c,16|11|0xffdc08", 参数.满级识别度,16,274,223,490}
		参数.狗粮满级.右一满级 = 操作.识别2(左一满级标识) --参数.狗粮满级中的左右，是根据换狗粮界面来说的
	end
	
	if 参数.狗粮表.中间 then
		local 中间满级标识={0x342312,"2|0|0xff9d1a,5|0|0x2d1f12,9|1|0xffa219,12|1|0xffa219,16|0|0x2a1d12,1|6|0xfbba11,9|5|0xffb713,13|4|0xffb114,2|12|0xefd207,6|11|0xfcd908,11|10|0xcfad0c,16|11|0xffdc08", 参数.满级识别度,456,243,634,485}
		参数.狗粮满级.中间满级 = 操作.识别2(中间满级标识)
	end
	
	if 参数.狗粮表.右一 then
		local 右一满级标识={0x342312,"2|0|0xff9d1a,5|0|0x2d1f12,9|1|0xffa219,12|1|0xffa219,16|0|0x2a1d12,1|6|0xfbba11,9|5|0xffb713,13|4|0xffb114,2|12|0xefd207,6|11|0xfcd908,11|10|0xcfad0c,16|11|0xffdc08", 参数.满级识别度,833,254,1054,493}
		参数.狗粮满级.左一满级 = 操作.识别2(右一满级标识)
	end
	
	if 参数.狗粮表.左二 then
		local 左二满级标识={0x342312,"2|0|0xff9d1a,5|0|0x2d1f12,9|1|0xffa219,12|1|0xffa219,16|0|0x2a1d12,1|6|0xfbba11,9|5|0xffb713,13|4|0xffb114,2|12|0xefd207,6|11|0xfcd908,11|10|0xcfad0c,16|11|0xffdc08", 参数.满级识别度,211,216,440,494}
		参数.狗粮满级.右二满级 = 操作.识别2(左二满级标识)
	end
	
	if 参数.狗粮表.右二 then
		local 右二满级标识={0x342312,"2|0|0xff9d1a,5|0|0x2d1f12,9|1|0xffa219,12|1|0xffa219,16|0|0x2a1d12,1|6|0xfbba11,9|5|0xffb713,13|4|0xffb114,2|12|0xefd207,6|11|0xfcd908,11|10|0xcfad0c,16|11|0xffdc08", 参数.满级识别度,628,238,846,514}
		参数.狗粮满级.左二满级 = 操作.识别2(右二满级标识)
	end
	keepScreen(false)
