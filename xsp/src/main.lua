...
if 参数.任务=='御魂' then
	...
	if results.御魂选择BOSS=='八歧大蛇' then 
		参数.任务='大蛇'
	elseif results.御魂选择BOSS=='卑弥呼' then
		参数.任务='卑弥呼'
...
if 参数.任务=='卑弥呼' then 
	------卑弥呼设置
	--选择层数
	参数.使用默认层数=results.卑弥呼使用默认层数.默认当前层数
	local tLayer = 
	{
		["一层"] = '壹', ["二层"] = '贰',
		["三层"] = '叁', ["四层"] = '肆',
	...
	参数.卑弥呼层数 = tLayer[results.卑弥呼层数]
	--组队方式
	参数.组队方式=results.卑弥呼组队方式
	参数.重设固定好友=results.卑弥呼重设固定队友.重设固定队友
	if 参数.组队方式=='3人队长' then 
		参数.队伍人数=3 
	elseif 参数.组队方式=='2人队长' then 
		参数.队伍人数=2 
	...
	--集火设置
	if results.卑弥呼是否点怪.点怪 then
		参数.卑弥呼_标记 = results.卑弥呼集火目标
	else
		参数.卑弥呼_标记 = 0
	end
	--标记己方
	if results.卑弥呼是否标记己方.标记己方 then
		参数.卑弥呼_标记己方 = results.卑弥呼标记己方
	else
		参数.卑弥呼_标记己方 = 0
	end
	----组队设置
	--队长设置
	if 参数.组队方式=='3人队长' or 参数.组队方式=='2人队长' then
		参数.队伍类型=results.卑弥呼队伍类型
		参数.房间邀请=results.卑弥呼是否邀请.邀请
		if 参数.房间邀请 then
			参数.房间邀请人数=results.卑弥呼邀请人数
			参数.房间邀请目标=results.卑弥呼邀请目标
		end
		参数.失败重邀=results.卑弥呼重邀.失败重邀
		参数.默认邀请队友=results.卑弥呼重邀.默认邀请队友
	end
	--队员设置
	if 参数.组队方式=='队员' then
		参数.默认接受邀请=results.卑弥呼自动接受.自动接受邀请
	end
end
--------------------
...
if 参数.任务=='探索' then
	...
	if 参数.探索组队类型~='队员' then
		...
		参数.BOSS已挑战=0
		--组队设置
		if results.探索组队类型 == '队长' then
			参数.队伍人数 = 3	--由于探索组队，房间界面没有3号位，脚本会认为3号已就位
			参数.队伍类型 = results.探索队伍类型
			参数.房间邀请 = results.探索是否邀请.邀请
			参数.默认邀请队友 = false
			if 参数.房间邀请 then
				参数.房间邀请人数=1
				参数.房间邀请目标=results.探索邀请目标
			end
...
if 参数.任务=='御灵' then
	...
	--标记己方
	if results.御灵是否标记己方.标记己方 then
		参数.御灵_标记己方 = results.御灵标记己方
	else
		参数.御灵_标记己方 = 0
	end
end
...
if 参数.交叉超鬼王 then
	...
	参数.超鬼王_不打公开 = results.超鬼王不打公开鬼王.不打公开鬼王
	--点怪
...
if 参数.交叉死神 then
	------死神
	--穿插设置
	参数.死神挤车=results.死神挤车.挤车
	参数.死神挤车开金币buff=results.交叉死神开金币buff.挤车时开启
	参数.死神自己发现开金币buff=results.交叉死神开金币buff.自己发现时开启
	if 参数.死神挤车 then
		参数.主任务=false
		参数.死神开始计时时刻=-1
	else
...
--	参数.寮突破间隔=10000
--	参数.个人突破间隔=10000
--	参数.死神开始计时时刻=9999999999999999999
	执行任务.设置流程()
	执行任务.启动任务()
